...
function Dark({ copied }) {
  const [colors, setColors] = useState([]);
  const uri = 'https://x-colors.yurace.pro/api/random/all?number=10&type=dark';
  const getColors = () => {
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-col items-center dark:text-black">
            <button
              onClick={getColors}
              className="dark:bg-neutral-800 hover:dark:bg-neutral-700 dark:text-gray-200 hover:bg-slate-400 transition-colors mt-[1%] w-[auto] p-5 h-[5vh] rounded-3xl bg-slate-300 font-bold flex items-center justify-center text-lg cursor-pointer mb-20"
            >
